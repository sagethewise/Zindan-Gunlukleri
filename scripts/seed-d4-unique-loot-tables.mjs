// scripts/seed-d4-unique-loot-tables.mjs
...
async function main() {
  ...
  // slug bazlı tekilleştirme (21000 hatasını engellemek için)
  ...
  const { error } = await supabase
    .from("d4_unique_loot_tables")
    .upsert(deduped, {
      onConflict: "slug",
    });

  if (error) {
    console.error("❌ Error upserting d4_unique_loot_tables:", error);
    process.exit(1);
  }

  console.log("✅ d4_unique_loot_tables upsert tamam.");
}
