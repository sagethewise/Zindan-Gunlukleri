...
async function main() {
  ...
  for (const item of items) {
    ...
    bySlug.set(item.slug, item); // aynı slug gelirse sonuncu kalır
  }
  ...
  if (error) {
    console.error("❌ Error upserting d4_tierlist:", error);
    process.exit(1);
  }

  console.log("✅ d4_tierlist upsert tamam.");
}
