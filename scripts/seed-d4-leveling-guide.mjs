// scripts/seed-d4-leveling-guide.mjs
...
async function main() {
  ...
  // slug bazlı tekilleştirme (21000 hatası yaşamayalım)
  ...
  const { error } = await supabase
    .from("d4_leveling_guides")
    .upsert(deduped, {
      onConflict: "slug",
    });

  if (error) {
    console.error("❌ Error upserting d4_leveling_guides:", error);
    process.exit(1);
  }

  console.log("✅ d4_leveling_guides upsert tamam.");
}
