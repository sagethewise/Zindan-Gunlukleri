// scripts/seed-d4-paragon-glyphs.mjs
...
async function main() {
  ...
  // slug bazlı tekilleştirme → 21000 hatasını engelle
  ...
  const { error } = await supabase
    .from("d4_paragon_glyphs")
    .upsert(deduped, {
      onConflict: "slug",
    });

  if (error) {
    console.error("❌ Error upserting d4_paragon_glyphs:", error);
    process.exit(1);
  }

  console.log("✅ d4_paragon_glyphs upsert tamam.");
}
